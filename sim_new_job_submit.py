...
class pars:
    MYDIR = '/w/halld-scshelf2101/halld3/home/ihossain/gr_dn_HRPPD_3.3_pixel_10KHz_out'
...
nArgs = len(sys.argv)
if nArgs != 1:
    print("Usage: python3 sim_new_job_submit.py")
    sys.exit()
...
def makeSLURMJob():
    ...
    while line:
        ...
        sim_command = "cd {0} && ./eicdirc -o ".format(pars.macrosTopDir) + outputFile + " -r 0 -theta {0} -x \"mix_pik\" -p 6.0 -w 0 -g 1 -c 4 -l 3 -trackingres 0.0005 -dn 116640000 -v 0 -gz 1 -ev 0 -b 1 -e {1}".format(inputTheta, pars.nEventsPerJob)
      ...
      #   slurmFile.write("if [ -d /u/scratch/" + getpass.getuser() + " ]; then\n") 
        slurmFile.write("if [ -d /scratch/" + getpass.getuser() + " ]; then\n") 
      #  slurmFile.write("    cd /u/scratch/" + getpass.getuser() + "\n")
     ...
     #   slurmFile.write("if command -v lsb_release &> /dev/null; then\n")
     #   slurmFile.write("    printf \"os: \"; /bin/lsb_release -a\n")
      #  slurmFile.write("else\n")
      #  slurmFile.write("    echo 'lsb_release not found'\n")
      #  slurmFile.write("fi\n")
 ...
 #       slurmFile.write(env_command + "\n")
  #      slurmFile.write("export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/site/12gev_phys/2.6/Linux_CentOS7.9.2009-gcc9.2.0/geant4/11.0.2/lib\n")
